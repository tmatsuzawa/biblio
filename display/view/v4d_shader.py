...
for name in ('sin', 'cos', 'tan', 'arcsin', 'arccos', 'arctan', 'arctan2', 'pi'): basic_namespace[name] = globals()[
    name]  # getattr(N, name)
...
def get_setup(filter='*.setup', search_dirs=[''], filter_vars={}, default_namespace=basic_namespace,
              first_line='**setup**', verbose=False, default={}, skip_filters=False, get_string=False):
    ...
    for dir in search_dirs:
        for fn in sorted(glob.glob(os.path.join(dir, filter))):
            ...
            line = get_line(f)
            if first_line and not line.startswith(first_line):
                continue
            else:
                line = get_line(f)
            ...
            while line.startswith('filter:'):
                ...
                try:
                    ...
                    print(
                        "Error evaulating '%s' in setup file '%s' -- aborting setup that passed filters!" % (line, fn))
                    continue
...
def show(source, s=slice(None), window_name='4d viewer', mem_buffer=False, window_kwargs={}):
...
def main():
    ...
    parser.add_argument('-r', dest='range', type=str, default=":",
                        help='range of frames to display, in python slice format')
    parser.add_argument('-m', dest='mem_buffer', type=bool, nargs='?', default=True, const=False,
                        help='Buffer all frames in OpenGL instead of unpacking each on demand -- not recommended unless you have an excess of video/system memory, and even then its only marginally faster.')
...
def set_uniforms(program, **vars):
    for key, val in vars.items():
        ...
        if val.dtype in ('u1', 'u2', 'u4', 'u8', 'i1', 'i2', 'i4', 'i8'):
            dt = int
        elif val.dtype in ('f', 'd'):
            dt = float
        else:
            raise ValueError('values for set_uniforms should be ints or floats')
...
class Image4D(object):
    ...
    def __init__(self, input, aspect_ratio=1., mem_buffer=False, load_setup=True, setup_dirs=['']):
        ...
        # Check if input is s4d of array
        if type(input) == str:
            self.filename = input
            ...
            if input.dtype == 'u1':
                ...
                raise ValueError(
                    'Input array type must be unsigned bytes or float/double (assumed 0-1 brightness scaling)')

        else:
            raise ValueError('Input type should be filename (s4d file) or array.')

        # Setup parameters (frame, display and color)
        ...
        if self.color_channels != 1:
            self.volume_size = self.volume_size[:-1]
    ...
    ##########################
        # mem_buffer set to False for MacBook (Mojave), True for iMac, Linux, and Windows
    ######################
        self.mem_buffer = False
        if self.mem_buffer:
            print('mem check t')
            self.gl_texture = glGenTextures(1)
            glBindTexture(GL_TEXTURE_3D, self.gl_texture)
            glTexImage3D(GL_TEXTURE_3D, 0, self.internal_format, self.volume_size[2], self.volume_size[1],
                         self.volume_size[0], 0, self.format, GL_UNSIGNED_BYTE,
                         test)
        else:
            print('mem check f')
            self.gl_texture = {}
    ######################
        if self.filename:
            setup_dirs.insert(0, os.path.split(self.filename)[0])
        ...
        if has_3dsetup:
            setup_info = self.header['3dsetup'].replace('print ', '')
            self.setup = eval_vals(setup_info)
            self.setup_file = '-- embedded in s4d --'
        else:
            self.setup_file, self.setup = get_setup('*.3dsetup',
                                                    setup_dirs, filter_vars,
                                                    default={'x_func': I3x, 'y_func': I3y, 'z_func': I3z})
    ...
    def gl_load_texture(self, frame_num):
        # tick()
        # print(self.gl_texture)
        if self.mem_buffer:
            print('gl texture', self.gl_texture)
            glBindTexture(GL_TEXTURE_3D, self.gl_texture)
        else:
            ## These lines appear to do nothing
            # if frame_num in self.gl_texture:
            #     return True
            self.gl_texture[frame_num] = glGenTextures(1)
            print('gl texture', self.gl_texture[frame_num])
        ...
        if self.mem_buffer:
            ...
            glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, self.volume_size[2], self.volume_size[1], self.volume_size[0],
                            self.format, GL_UNSIGNED_BYTE, buffer)
        else:
            glTexImage3D(GL_TEXTURE_3D, 0, self.internal_format, self.volume_size[2], self.volume_size[1],
                         self.volume_size[0], 0, self.format, GL_UNSIGNED_BYTE, buffer)
...
class FuncSetter(object):
    ...
    def __set__(self, obj, val):
        try:
            if self.glut_func is not None:
                self.glut_func(val)
            else:
                print('Warning: %s property is ignored.' % self.prop_name)
        except:
...
class GLUTWindow(object):
                ...
                void main() {
                    ...
                    if (l < max_length) {
                        for (float ll = 0.5 * step_size; ll < l; ll += step_size) {
                            ...
                            intensity += c*c * opacity * (1.0-intensity.a); 
                            p += s;
                        }
                    }
    ...
    def screenshot(self, fn=None, alpha=False):
        ...
        glutSwapBuffers()
        glFinish()  # Make sure drawing is complete

        glReadBuffer(GL_FRONT)
        img = glReadPixels(0, 0,
                           self.width,
                           self.height,
...
def innermost_draw(Rv, r=True, g=True, b=True):  # (:
    ...
    set_uniforms(self.volume_shader, brightness=float(BRIGHTNESS) / float(OPACITY), opacity=float(OPACITY),
                 **DATA.shader_vars)
...
def draw_scene():
    global FOV, LAST_TIME, CURRENT_FRAME, PLAYING, EYE_SPLIT, FRAMES, R, \
        ...
        COLOR_SCALE, COLOR_CYCLES, EDGES, BACKGROUND_COLOR, GREEN_MAGENTA, PERSPECTIVE_CORRECT
    ...
    display_settings = (FOV, int(CURRENT_FRAME), EYE_SPLIT, list(map(tuple, R)), DISPLAY_3D,
                        ...
                        COLOR_SCALE, COLOR_CYCLE, LINE_THICKNESS, list(EDGES.flatten()), PERSPECTIVE_CORRECT)
...
EDGE_ADJUST = (b'Q', b'A', b'W', b'S', b'E', b'D', b'R', b'F', b'T', b'G', b'Y', b'H')
...
def key_pressed(*args):
    global BRIGHTNESS, FRAME_RATE, PLAYING, DISPLAY_3D, EYE_SPLIT, FULLSCREEN, \
        ...
        PRINT_FRAMERATE, COLOR_SCALE, COLOR_CYCLE, LINE_THICKNESS, \
    ...
    if args[0] == ESCAPE:
        ...
        EYE_SPLIT /= 2 ** (1. / 8)
        print(b"Total eye shear: %.3f" % (EYE_SPLIT * 2))
    elif args[0] in (']', '}'):
        EYE_SPLIT *= 2 ** (1. / 8)
        print(b"Total eye shear: %.3f" % (EYE_SPLIT * 2))
    elif args[0] in (b'\\', b'|'):
        ...
        if MOVIE_FILE is None:
            ...
            if LAST_MOVIE_FRAME == cf:
                frames = 10
            else:
                frames = abs(cf - LAST_MOVIE_FRAME)
