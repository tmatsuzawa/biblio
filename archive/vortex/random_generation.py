...
def noise(base, sigma, n, T=0, N=None, remove_mean=True):
    """
    ...
    path = np.transpose(np.asarray(
        [np.sum([base[:, j] + An[i, j] * np.power(np.sin((i + T) * theta), 1) for i in range(n)], axis=0) for j in
         range(3)]))
...
def distribution(sigma, n, display=False):
    ...
    for p in range(n_p):
        ...
        if p < 3:
            savename = './Random_path/Tests/Examples/sigma_' + str(round(sigma * 1000)) + 'm_n' + str(n) + '_' + str(
                p + 1)
            save(t, prefix=savename)
...
def V_distribution2(n, sigma, fignum=1):
    ...
    names = {'sigma': sigma, 'n': n,
             'Np': N_p}  # name:eval(name) for name in ['sigma','n','N_p']}# this piece of code does not work systematically
    ...
    graphes.save_figs(figs, prefix='Random_path/V_Distributions/Compilation/', suffix=suf(names), dpi=300, display=True,
                      frmt='png')


def vfield(sigma, modes, fignum=1):
    names = {'sigma': sigma,
             'n': modes}  # name:eval(name) for name in ['sigma','n','N_p']}# this piece of code does not work
    ...
    graphes.save_figs(figs, prefix='Random_path/V_Distributions/Fields/', suffix=suf(names), dpi=300, display=True,
                      frmt='png')
