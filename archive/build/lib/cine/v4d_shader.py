...
try:
    ...
    from stephane.cine import Sparse4D
...
#        glClearColor(1.0, 1.0, 1.0, 0.0) 
    ...
    def screenshot(self, fn=None, alpha=True):
        if alpha:
            channels = "RGBA"
            glc = GL_RGBA
        else:
...
def movie_draw_scene():
    ...
    try: #Catch exceptions and exit, as there is no keyboard control    
        global WINDOW, SEQUENCE, CURRENT_FRAME, DISPLAY_3D, Z_SHIFT, R, BRIGHTNESS, Z_STEP, DRAW_BOX, FRAME_COUNT, EXPORT_DIR, FOV, EDGES, BACKGORUND_COLOR
...
#    colorMask()
    ...
    set_uniforms(self.volume_shader, brightness=float(BRIGHTNESS) / float(OPACITY), opacity=float(OPACITY), **DATA.shader_vars)
...
#    set_uniforms(self.volume_shader, brightness=float(BRIGHTNESS))
    ...
    if DRAW_BOX:
        ...
        #Draw a black background behind everything
