...
def crawl(l, ext=None):
    ...
    for fn in l:
        if os.path.isdir(fn):
            files += [os.path.join(fn, x) for x in crawl(os.listdir(fn), ext)]
        else:
...
removed = []
files_lower = [os.path.splitext(x)[0] +  os.path.splitext(x)[1].lower() for x in files]
...
for fn in files:
    ...
    #print header
    print((args.range))
    frames = eval('x[%s]' % args.range, {'x':list(range(num_frames))})
