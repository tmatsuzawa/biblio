...
def convert_file(ifn, ofn=None, clip=0, max_frame=None, overwrite=False):
    if ofn is None:
        outfyle = os.path.splitext(ifn)[0] + '.sparse'
    else:
        fyle, name = os.path.split(ifn)
...
#    sys.stdout.flush()
    ...
    s = cine.Sparse(outfyle, 'w', header, max_blocks=max_frame)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', dest='ofl', type=str, default=None, help='output directory')
    ...
    if len(fns) == 1:
        convert_file(fns[0], ofn=args.ofl, clip=args.clip, overwrite=args.overwrite)
