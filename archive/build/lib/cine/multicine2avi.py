...
class FrameGenerator(object):
    def __init__(self, fn, input_args):
        ...
        if '[' in fn and fn[-1] == ']':
            fn, s = fn.split('[')
            try:
                self.frame_slice = slice(*list(map(noneint, s[:-1].split(':'))))
            except:
                raise ValueError("Couldn't convert '[%s' to slice notation for file '%s'" % (s, fn))
        
        self.fn = fn
        self.source = cine.Cine(fn)
        self.frames = list(range(*self.frame_slice.indices(len(self.source))))
    ...
    def __getitem__(self, key):
        if type(key) == slice:
            return list(map(self.__getitem__, list(range(self.image_count))[key]))
...
fns = [os.path.splitext(x)[0] for x in args.cines]
for i in range(min(list(map(len, fns)))):
    broken = False
    for j in range(len(fns) - 1):
        if fns[j][i] != fns[j+1][i]:
            ...
            break
    if broken: break
basename = fns[0][:i] + '_AND_'.join([x[i:] for x in fns])

        
frame_slice = slice(None)
try:
    frame_slice = slice(*list(map(noneint, args.frames.split(':'))))
except:
...
frames = list(range(*frame_slice.indices(max_frames)))
N = len(frames)
