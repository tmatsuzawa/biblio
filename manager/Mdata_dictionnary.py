# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 16:55:10 2015
...
def generate_dictionnary():
    ...
    for rootDir in file_architecture.Dir_data:
        ...
        for Dir in dataDirList:
            ...
            print(date)
            failure += Sdata_manip.Sdata_gen_day(date)
...
def write(dictList,Dir,date):
    ...
    if len(dictList)>0:
        ...
        if not os.path.isdir(SaveDir):
            os.makedirs(SaveDir)
            file=SaveDir+'/Experiment_overview_'+date+'.txt'
    
            print(dictList)
        rw_data.write_dictionnary(file,key_dict,dictList)
