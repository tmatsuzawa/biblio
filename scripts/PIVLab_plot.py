...
def load_matlab_data(datapath):
    ...
    if j == 4:
        x, y, ux, uy, omega = [np.asarray(z) for z in [x, y, ux, uy, omega]]
        return x, y, ux, uy, omega
    elif j == 3:
        x, y, ux, uy = [np.asarray(z) for z in [x, y, ux, uy]]
        return x, y, ux, uy
...
if __name__ == '__main__':
    ...
    parser.add_argument('-plot_spacing', dest='plot_spacing', type=int, default=20)
    ...
    for cnum, cine_file in enumerate(cine_inputs):
        ...
        trForVr, vxr, vyr, vr =[], [], [], []
        trForDr, dr = [], []
        trForGammar, gammar, cxmaxlist, cymaxlist, cxminlist, cyminlist = [], [], [], [], [], []
        cxlist, cylist = [], []
        time = []

        # LOAD CINsE FILE AND GRAB IMAGE SIZE
        ...
        resultsdir = date_dir + '/PIV_AnalysisResults_spacing80/' + name  #ABS PATH TO OUTPUT DIRECTORY
        # resultsdir = '/Users/stephane/Documents/Takumi/Analysis_Results/' + name

        try:
            span = float(browse.get_string(name, 'piston', 'mm_'))
        except ValueError:
        ...
        if args.stop == 0:
            frames = list(range(args.start, movie.__len__(), args.skip))
        else:
        ...
        for i, f in enumerate(frames):
            ...
            if f - ((f / plot_spacing) * plot_spacing) == 0:
                ...
                #     # x,y, ux, uy, omega are in the units of px and frame (NOT mm and sec)
                #     if omegabool:
                ...
                xmin, xmax, ymin, ymax = min(Xgrid.flatten()), max(Xgrid.flatten()), min(Ygrid.flatten()), max(
                    Ygrid.flatten())
                ...
                try:
                    ...
                    cx, cy = Xgrid_around_max[max_xw, max_yw], Ygrid_around_max[max_xw, max_yw]
                    #cxmax, cymax = Xgrid_around_max[max_xw, max_yw], Ygrid_around_max[max_xw, max_yw]
                    #cxmin, cymin = Xgrid_around_min[min_xw, min_yw], Ygrid_around_min[min_xw, min_yw]
                ...
                bad_x_path = False
                path_x_limits = radii[-1] * np.cos(phi) + cx
                bad_y_path = False
                path_y_limits = radii[-1] * np.sin(phi) + cy
                ...
                print(len(trForVr), trForVr)
                if len(trForVr) > 1:
                    # Calculate ring velocity
                    vr.append(compute_dist(cxlist[counter], cylist[counter], cxlist[counter-1], cylist[counter-1])\
                              / (trForVr[counter]-trForVr[counter-1]))
                ...
                counter = counter + 1
                #Append a reasonable circulation value

                # graph.show()
        #Clean data about circulation, ring diameter, ring velocity
        trForVr = trForVr[:-1]
        trForGammar, gammar = process.clean_multi_dim_array_pair_using_median(trForGammar, gammar, cutoffratio=0.6, mode='less')
        trForGammar, gammar = process.clean_multi_dim_array_pair_using_median(trForGammar, gammar, cutoffratio=1.3, mode='greater')
        ...
        if gammar_avg*1.3 > np.max(gammar):
            graph.setaxes(ax8, time[0], time[-1], 0, gammar_avg*1.3)
        else:
           graph.setaxes(ax8, time[0], time[-1], 0, np.max(gammar))
        text = 'Average $\Gamma$: %.1f' % gammar_avg + '$mm^2/s$'
        graph.addtext(ax8, x=(time[-1]-time[0])/10., y=gammar_avg*1.3/4.*2, text=text)
        text = 'Std.: %.1f' % gammar_std + '$mm^2/s$'
        graph.addtext(ax8, x=(time[-1] - time[0]) / 10., y=gammar_avg * 1.3 / 4., text=text)
        ...
        if vr_avg*1.3 > np.max(vr):
            graph.setaxes(ax9, time[0], time[-1], 0, vr_avg*1.3)
        else:
            graph.setaxes(ax9, time[0], time[-1], 0, np.max(vr))
        text = 'Average $v_r$: %.1f' % vr_avg + '$mm/s$'
        graph.addtext(ax9, x=(time[-1]-time[0])/10., y=vr_avg*1.3/4.*2, text=text)
        text = 'Std.: %.1f' % vr_std + '$mm/s$'
        graph.addtext(ax9, x=(time[-1] - time[0]) / 10., y=vr_avg * 1.3 / 4., text=text)
        ...
        if dr_avg*1.3 > np.max(dr):
            graph.setaxes(ax10, time[0], time[-1], 0, dr_avg*1.3)
        else:
            graph.setaxes(ax10, time[0], time[-1], 0, np.max(dr))
        text = 'Average $D$: %.1f' % dr_avg + '$mm$'
        graph.addtext(ax10, x=(time[-1]-time[0])/10., y=dr_avg*1.3/4.*2, text=text)
        text = 'Std.: %.1f' % dr_std + '$mm$'
        graph.addtext(ax10, x=(time[-1] - time[0])/10., y=dr_avg * 1.3 / 4., text=text)
