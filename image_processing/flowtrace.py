...
def flowtrace(image_dir, frames_to_merge, out_dir='same', use_parallel=True, max_cores=8, frames_to_skip=1, ext='tif',
              **kwargs):
    '''
    ...
    '''
    image_files = glob.glob(image_dir + '/*.' + ext)
    ...
    if use_parallel and parallel_available:
        ...
        processes = [
            mp.Process(target=sliding_zproj_internal, args=(chunks_list[ii], frames_to_merge, out_dir), kwargs=kwargs)
