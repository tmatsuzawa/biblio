...
try:
    ...
    import vector  # Allows it to be run from package dir.
try:
    from . import simple_cl
except:
    ...
    if arr.shape == new_shape:  # Shortcut
        ...
        for so, st in zip(arr.shape, new_shape):
            if so == st:
                ...
                raise ShapeError('input not castable into array of shape %s' % shape_str(shape))
...
class Mesh(object):
    ...
    def scale(self, s):
        ...
        ----------
        s : float or vector
            scale factor for the mesh, to be done in place
        """
        self.points *= s
    ...
    def __iadd__(self, other):
        if hasattr(other, 'points') and hasattr(other, 'triangles'):
            ...
            if shc or ohc:
                if not ohc:
                    # warnings.warn('trying to add two meshes when only one has colors defined; making uncolored mesh
                    #  white', MeshWarning, stacklevel=2)
                ...
                if p1 is not None:
                    if p2 is not None:
                        ...
                        if prop == 'normals':
                            ...
                            self.attached[prop] = p_new
                    else:
                        # warnings.warn('trying to add two meshes when only one has normals defined; dropping normals
                        # from result', MeshWarning, stacklevel=2)
                        missing_props.append(prop)
                        if prop == 'normals':
                            del self.normals
                        else:
                            del self.attached[prop]

                elif p2 is not None:
                    # warnings.warn('trying to add two meshes when only one has normals defined; dropping normals from
                    #  result', MeshWarning, stacklevel=2)
                    missing_props.append(prop)

            if missing_props:
                warnings.warn(
                    'tried to add two meshes with different attached properties (conflicted values: %s)\nmissing '
                    'properties dropped from result' % missing_props)
    ...
    def clip(self, a, level=0, both_sides=False):
        ...
        kw = dict(
            triangles=np.concatenate(
                [point_map[self.triangles[ai_ti]], np.asarray(new_tri, dtype='i8').reshape(-1, 3)]),
            attached={})
        ...
        if both_sides:
            ...
            kw_out = dict(
                triangles=np.concatenate(
                    [point_map[self.triangles[ao_ti]], np.asarray(new_tri_out, dtype='i8').reshape(-1, 3)]),
                attached={})
    ...
    def color_by_proximity(self, p, c=None, w=None, rgb_only=True, power=-6, period=None):
        ...
        if c.shape[-1] == 3:
            c = np.concatenate([c, np.zeros((len(c), 1))], axis=-1)
        elif c.shape[-1] != 4:
            raise ValueError('last dimension of color array must be 3 or 4')

        if w is None:
            w = np.ones((len(c), 1))
        else:
            w = enforce_shape(w, (len(c), 1))
...
def load_mesh(fn, file_type=None):
    ...
    if file_type.lower() == 'ply':
        ...
        try:
            points = np.array([data['vertex'][k] for k in ('x', 'y', 'z')]).T
        except:
            raise PLYError('ply file %s has missing or invalid vertex position data' % fn)

        try:
            triangles = data['face']['vertex_indices']
        except:
            raise PLYError('ply file %s has missing or invalid triangle data' % fn)
    ...
    while line != 'end_header':
        ...
        if m:
            ...
            current_element.append(
                (m.group(3), (format_char + PLY_PROPERTY_TYPES[tn], format_char + PLY_PROPERTY_TYPES[tl])))
            continue
        ...
        if fast_decode:
            ...
            def conv(t, x):
                try:
                    ...
                    raise PLYError('expected %s, found "%s" (in file %s)' % (t, x, repr(f)))
            ...
            for i in range(ne):
                if format_type == 'ascii':
                    ...
                    for name, t in py_types:
                        if not parts:
                            raise PLYError(
                                'when decoding %s #%s, not enough items in line (in file %s)' % (e, i, repr(f)))
                        if type(t) in (list, tuple):
                            ...
                            if len(parts) >= nl:
                                ...
                                raise PLYError(
                                    'when decoding %s #%s, not enough items for list (in file %s)' % (e, i, repr(f)))
                        else:
...
def encode_ply(points, tris, normals=None, colors=None, enforce_types=True, extra_vertex_attributes=None):
    ...
    **Note:** if you wish to convert a :class:`Mesh` object to a "ply" file, use
    ...
    for k, v in extra_vertex_attributes.items():
        ...
        if v.shape != (N,):
            raise PLYError(
                'extra vertex attributes must have same length as number of vertices\n(attribute %s has shape %s, not %s)' % (
                k, v.shape, (N,)))
