...
ICUML inclined crack under mixed loading
...
List of Predefined Surfaces (options for string variable surface)
-----------------------------------------------------------------
===========  ============
Surface      Description
===========  ============
...
def planestress2strain_dolf(stress, E, nu, alph, T):
    ...
    strain = (1. + nu) / E * (stress - nu / (1. + nu) * dolf.tr(stress) * dolf.Identity(2)) + alph * T * dolf.Identity(
        2)
    return strain
...
def restart_refine(ind, rrIND, N, x, y, phi, outdir, params, dN=20000, n_clearance=30):
    """ Restart simulation at previous, saved time-step. Previous step saved in <rundir>/solution/

    Parameters
    ----------
    ind : int
        current index (iteration #) at which time simulation is told to revert to previous saved config
    rrIND : int
        how many times simulation has restarted and refined mesh already (initially zero)
    phi : dolfin function defined as VectorFunction on current (unrefined) mesh
        the current dolfin object for the phase
    outdir : string
        the simulation run directory
    params : dict
        parameters of the problem
    dN : int
        amount by which to increase # pts in mesh
    n_clearance : int

    Returns
    ----------
    ind,rrIND,t,N,Vt,Vv,Vf,x,y,u,phi,h,bcu,u_0,P,alph,arngmt,au,Lu,g,gprime,epsilon_k,Es_phi,aphi,Lphi,ds : various
        all necessary objects for updated problem
    """
    philist = sorted(glob.glob(outdir + 'solution/phi_*.xml'))
    ...
    # New mesh
    N += dN
    mesh, meshd, meshfile = genmesh(params['shape'], params['mesht'], N, params['xi'], params['theta'],
                                    0.5 * params['L'], params['eta'])
    params[key + '_N'] = N
    params[key + '_meshd'] = meshd
    params[key + '_meshfile'] = meshfile
    ...
    lookupXYZ = np.dstack((x, y, phiv))[0]
    ...
    u, phi, h, bcu, u_0, P, alph, arngmt, au, Lu, g, gprime, epsilon_k, Es_phi, aphi, Lphi, ds = \
        quasistatic_KKL_PDEs_iterative(mesh, Vv, Vf, phi_k, params['BCUP'], params['BCtype'], params['E'], params['nu'], \
                                       ...
                                       params['Ec'], params['kappa'], params['chi'], \
                                       params['dt'], alph=params['alph'], shape=params['shape'], \
                                       tensile=params['tensile'], A=params['A'])
    # save updated parameters file
    write_parameters(outdir + 'parameters.txt', params)

    return ind, rrIND, t, N, Vt, Vv, Vf, x, y, u, phi, h, bcu, u_0, P, alph, arngmt, au, Lu, g, gprime, epsilon_k, \
           Es_phi, aphi, Lphi, ds
...
def apply_bcu(Vv, BCtype, u_0, R, xc, yc, N, shape):
    ...
    def boundary_sides(x, on_boundary):
        ...
        if N > 60000:
            if shape == 'square':
                ...
                return on_boundary and (x[0] < -R * (0.98) or x[0] > R * (0.98)) and x[1] < 2 * R * 0.99 and x[
                                                                                                                 1] > -2 * R * 0.99
            elif shape == 'rectangle1x2':
                return on_boundary and (x[0] < -2 * R * (0.98) or x[0] > 2 * R * (0.98)) and x[1] < R * 0.99 and x[
                                                                                                                     1] > -R * 0.99
        elif N < 20000:
            if shape == 'square':
                return on_boundary and (x[0] < -R * (0.96) or x[0] > R * (0.96)) and x[1] < R * 0.99 and x[
                                                                                                             1] > -R * 0.99
            elif shape == 'rectangle2x1':
                return on_boundary and (x[0] < -R * (0.96) or x[0] > R * (0.96)) and x[1] < 2 * R * 0.99 and x[
                                                                                                                 1] > -2 * R * 0.99
            elif shape == 'rectangle1x2':
                return on_boundary and (x[0] < -2 * R * (0.96) or x[0] > 2 * R * (0.96)) and x[1] < R * 0.99 and x[
                                                                                                                     1] > -R * 0.99
        else:
            if shape == 'square':
                return on_boundary and (x[0] < -R * (0.98) or x[0] > R * (0.98)) and x[1] < R * 0.99 and x[
                                                                                                             1] > -R * 0.99
            elif shape == 'rectangle2x1':
                return on_boundary and (x[0] < -R * (0.98) or x[0] > R * (0.98)) and x[1] < 2 * R * 0.99 and x[
                                                                                                                 1] > -2 * R * 0.99
    ...
    # APPLY BOUNDARY CONDITION
    if BCtype in ['uniaxialmixedfree_u1s1', 'uniaxialmixedfree_uvals1', 'uniaxialmixedfree_u1sval']:
...
def def_bcP(BCtype, U, E, R=0.06, xc=0., yc=0.):
    ...
    if BCtype == 'uniaxial' or 'uniaxial-PURL' or BCtype == 'Usingleptcorner_Puniaxial':
        ...
        P = dolf.Expression(('E*U*sqrt( pow(x[0]-xc,2)+pow(x[1]-yc, 2) )*cos(atan2(x[1]-yc,x[0]-xc))/R',
                             'E*U*sqrt( pow(x[0]-xc,2)+pow(x[1]-yc, 2) )*sin(atan2(x[1]-yc,x[0]-xc))/R'), U=U, E=E,
                            xc=xc, yc=yc, R=R)
    return P
...
def initialPhase_Ncrack(xy, spacing, Number, Y, beta, W, a, xi, fallofftype='polygauss'):
    """Set initial phase to have a row of slits along the line Y=const, with same orientation.
    Row of slits has even spacing and a center of mass at x=0.

    Parameters
    ----------
    xy : N x 2 array
        positions of points for which to evaluate phase
    spacing : float
        distance between slits in the row of slits
    Number : int > 1
        number of starting slits
    Y : float
        Y position of the row of starting slits (centers)
    beta : float
        orientation of the starting slits
    W : float
        characteristic width of the starting slits
    a : float
        initial half-length of slits
    fallofftype : string ('polygauss' 'linear')
        describes the way that the phase falls to 1 near initial slit
    """
    ...
    linesegs = np.zeros((Number, 4))
    for i in range(Number):
        linesegs[i] = [H[i] - a * np.cos(beta), Y - a * np.sin(beta), \
                       H[i] + a * np.cos(beta), Y + a * np.sin(beta)]

    # print 'xy = ', xy
    dist = mindist_from_multiple_linesegs(xy, linesegs)
    ...
    elif fallofftype == 'polygauss':
        closeIND = (dist <= 3.9 * W)
        val[closeIND] = 1 - C * np.exp(-(dist[closeIND] / xi) ** 2 / (2 * polysig ** 2)) * (
            1 + polya * (dist[closeIND] / xi) ** 2 + polyb * (dist[closeIND] / xi) ** 4)
    return val


def initialPhase_vec(xy, H, Y, beta, W, a, xi, fallofftype='polygauss', polysig=0.565, C=0.966, polya=-0.392,
                     polyb=0.638):
    """Initial phase for a sheet with many cracks of length a and width W.
    H,Y, and beta are vectors or lists of crack positions and orientations.

    Parameters
    ----------
    xy : array or list
        points for which to evaluate initial phase
    H : float or array
        x position(s) of the center(s) of the crack(s)
    Y : float or array
        y position(s) of the center(s) of the crack(s)
    beta : float
        orientation angle(s) of the crack(s)
    W : float
        characteristic width of the starting slits
    a : float or array
        half length of the crack (distance from center of crack to tip)
    xi : float
        intrinsic process zone size; used to estimate fall-off width of initial phase profile near crack
    fallofftype : string ('polygauss' or 'linear')
        shape of the phase profile near the crack
    C, polya, polyb : floats
        fitting parameters for initial phase profile near the crack

    Returns
    ----------
    val : array
        initial phase values between zero and 1.0
    """
    # Determine number of cracks (Ncracks)
    if not np.shape(H) and not np.shape(Y) and not np.shape(beta):
        # Ncracks = 1
        # Handle single crack case
        linesegs = [[H - a * np.cos(beta), Y - a * np.sin(beta), H + a * np.cos(beta), Y + a * np.sin(beta)]]
    else:
        if np.shape(H):
            Ncracks = len(H)
            ...
            if isinstance(beta, float):
                beta = beta * np.ones_like(H)
        elif np.shape(Y):
            Ncracks = len(Y)
            H = H * np.ones_like(Y)
            if isinstance(beta, float):
                beta = beta * np.ones_like(Y)
        else:
            Ncracks = len(beta)
            H = H * np.ones_like(beta)
            Y = Y * np.ones_like(beta)

        if isinstance(a, float):
            a = a * np.ones_like(H)

        # Build endpoint vectors for each crack
        linesegs = np.zeros((Ncracks, 4))
        for i in range(Ncracks):
            linesegs[i] = [H[i] - a[i] * np.cos(beta[i]), Y[i] - a[i] * np.sin(beta[i]), \
                           H[i] + a[i] * np.cos(beta[i]), Y[i] + a[i] * np.sin(beta[i])]
...
def surf_vec(xy, surf, R=0.06, alph='default', x0='default', xc=0., yc=0.):
    """ Definition of surface vector using numpy"""
    if surf == 'flat':
        if alph == 'default':
            alph = 0.000
        x0 = (1. / 2.35) * R
        fv = np.zeros_like(xy[:, 0])
        arrangement = [[]]
    elif surf == 'bump':
        alph = 0.706
        x0 = 0.2 * R
        alph = alph;
        sigma = x0;
        fv = alph * sigma * np.exp(-0.5 * ((xy[:, 0] - xc) / sigma) ** 2 \
                                   - 0.5 * ((xy[:, 1] - yc) / sigma) ** 2)
        arrangement = [[xc, yc]]
    elif surf == 'bumps2x2':
        bspace = R * 0.3
        ...
        sig0 = x0[0];
        sig1 = x0[1]
        fv = alph0 * sig0 * exp(-0.5 * (pow((x[0] - mux0) / sig0, 2)) - 0.5 * (pow((x[1] - muy0) / sig0, 2))) \
             + alph1 * sig1 * exp(-0.5 * (pow((x[0] - mux1) / sig1, 2)) - 0.5 * (pow((x[1] - muy1) / sig1, 2)))
        arrangement = np.array([[mux0, muy0], [mux1, muy1]])
    elif surf == 'spherical_cap':
        # x0 = radius of sphere, alph = R/x0, R is width of sample
        #   ________  s = sqrt(x**2+y**2)
        # --   |    --
        #      |    /  --
        #  x0  |  /      --
        #      |/
        ...
        if x0 == 'default':
            x0 = 2 * R
        alph = R / x0
        s = np.sqrt((xy[:, 0] - xc) ** 2 + (xy[:, 1] - yc) ** 2)
        theta = s / x0
        fv = x0 * np.cos(theta)
        arrangement = [[xc, yc]]

    return alph, x0, fv, arrangement


def surf_geom():
    """ Definitions of geometries, kept on hand purely for reference, since this function
    does not actually run (requires dolfin structure or class structure, which this module does not assume)"""
    aval = 0.6
    if surf == 'flat':
        alph = 0.000
        x0 = (1. / 2.35) * R
        ...
        alph = [0.30, 0.30, 0.30]
        # x0 = [(1./2.35)*R,(1./2.35)*R]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.6 * R
        mux0 = 0.0
        mux1 = 0.0
        mux2 = 0.0
        ...
        muy1 = 0.0
        muy2 = bspace
        f = Expression("alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/sig0, 2)))  "
                       "+alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/sig1, 2)))  "
                       "+alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/sig2, 2)))  ",
                       alph0=alph[0], alph1=alph[1], alph2=alph[2],
                       mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2,
                       sig0=x0[0], sig1=x0[1], sig2=x0[2])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2]]
    elif surf == 'bumpavoid3_ar3_theta45':
        alph = [aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        dspace = 0.8 * x0[0]
        ...
        f = Expression("alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/sig0, 2)))  "
                       "+alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/sig1, 2)))  "
                       "+alph2*sig2*exp(-0.5*(pow(((x[0] - mux2)*cos(t)+(x[1]-muy2)*sin(t))/sig2, 2)) "
                       "                -0.5*(pow(((x[1] - muy2)*cos(t)-(x[0]-mux2)*sin(t))/(3*sig2), 2)))  ",
                       t=np.pi * 0.25, alph0=alph[0], alph1=alph[1], alph2=alph[2],
                       mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2,
                       sig0=x0[0], sig1=x0[1], sig2=x0[2])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2]]
    elif surf == 'bumpavoid4_ar3_theta45':
        alph = [aval, aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        dspace = 0.8 * x0[0]
        mux0 = -dspace
        mux1 = dspace
        ...
        mux3 = -0.5 * dspace
        basey = -2 * R + 0.2 * R
        muy0 = basey + 0.5 * bspace
        muy1 = basey + 1.5 * bspace
        muy2 = basey + 3.0 * bspace
        muy3 = basey + 3.8 * bspace
        f = Expression("alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/sig0, 2)))  "
                       "+alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/sig1, 2)))  "
                       "+alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/sig2, 2)))  "
                       "+alph3*sig3*exp(-0.5*(pow(((x[0] - mux3)*cos(t)+(x[1]-muy3)*sin(t))/sig3, 2)) "
                       "                -0.5*(pow(((x[1] - muy3)*cos(t)-(x[0]-mux3)*sin(t))/(3*sig3), 2)))  ",
                       t=np.pi * 0.25, alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3],
                       mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2, mux3=mux3, muy3=muy3,
                       sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3]]
    elif surf == 'bumpavoid4_ar3_theta45_max':
        alph = [aval, aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        dspace = 0.8 * x0[0]
        ...
        mux3 = -0.5 * dspace
        basey = -2 * R + 0.2 * R
        muy0 = basey + 0.8 * bspace
        muy1 = basey + 1.5 * bspace
        muy2 = basey + 3.0 * bspace
        muy3 = basey + 3.8 * bspace
        f = Expression(
            "max(max(max(alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/sig0, 2))),  "
            "alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/sig1, 2)))),  "
            "alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/sig2, 2)))),  "
            "alph3*sig3*exp(-0.5*(pow(((x[0] - mux3)*cos(t)+(x[1]-muy3)*sin(t))/sig3, 2)) "
            "                -0.5*(pow(((x[1] - muy3)*cos(t)-(x[0]-mux3)*sin(t))/(3*sig3), 2))))  ",
            ...
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3]]
    elif surf == 'bumpavoid4_ar2_theta45_max':
        alph = [aval, aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        dspace = 0.8 * x0[0]
        ...
        f = Expression(
            ...
            "alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/sig2, 2)))),  "
            "alph3*sig3*exp(-0.5*(pow(((x[0] - mux3)*cos(t)+(x[1]-muy3)*sin(t))/sig3, 2)) "
            "                -0.5*(pow(((x[1] - muy3)*cos(t)-(x[0]-mux3)*sin(t))/(2*sig3), 2))))  ",
            t=np.pi * 0.25, alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3],
            mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2, mux3=mux3, muy3=muy3,
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3]]
    elif surf == 'bumpavoid5_ar2_theta45_max':
        alph = [aval, aval, aval * 0.8, aval * 0.8, aval]
        ...
        muy1 = basey + 1.5 * bspace
        muy2 = basey + 3.0 * bspace
        muy3 = basey + 3.2 * bspace
        muy4 = basey + 3.8 * bspace
        f = Expression(
            "max(max(max(max(alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/sig0, 2))),  "
            "alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/sig1, 2)))),  "
            "alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/sig2, 2)))),  "
            "alph3*sig3*exp(-0.5*(pow((x[0] - mux3)/sig3, 2)) -0.5*(pow((x[1] - muy3)/sig3, 2)))),  "
            "alph4*sig4*exp(-0.5*(pow(((x[0] - mux4)*cos(t)+(x[1]-muy4)*sin(t))/sig4, 2)) "
            "                -0.5*(pow(((x[1] - muy4)*cos(t)-(x[0]-mux4)*sin(t))/(2*sig4), 2))))  ",
            t=np.pi * 0.25, alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3], alph4=alph[4],
            mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2,
            mux3=mux3, muy3=muy3, mux4=mux4, muy4=muy4,
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3], sig4=x0[4])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3], [mux4, muy4]]
    elif surf == 'bumpavoid5max':
        alph = [aval, aval, aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        dspace = 0.8 * x0[0]
        mux0 = -dspace
        ...
        f = Expression(
            ...
            "alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/sig2, 2)))),  "
            "alph3*sig3*exp(-0.5*(pow((x[0] - mux3)/sig3, 2)) -0.5*(pow((x[1] - muy3)/sig3, 2)))),  "
            "alph4*sig4*exp(-0.5*(pow((x[0] - mux4)/sig4, 2)) -0.5*(pow((x[1] - muy4)/sig4, 2))))  ",
            t=np.pi * 0.25, alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3], alph4=alph[4],
            mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2,
            mux3=mux3, muy3=muy3, mux4=mux4, muy4=muy4,
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3], sig4=x0[4])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3], [mux4, muy4]]
    elif surf == 'bumps1x4aspect2add':
        alph = [aval, aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        mux0 = 0.0
        ...
        muy2 = 0.5 * bspace
        muy3 = 1.5 * bspace
        f = Expression("alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/(2*sig0), 2)))  "
                       "+alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/(2*sig1), 2)))  "
                       "+alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/(2*sig2), 2)))  "
                       "+alph3*sig3*exp(-0.5*(pow((x[0] - mux3)/sig3, 2)) -0.5*(pow((x[1] - muy3)/(2*sig3), 2)))  ",
                       alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3],
                       mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2, mux3=mux3, muy3=muy3,
                       sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3]]
    elif surf == 'bumps1x4aspect2max':
        alph = [aval, aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = R
        mux0 = 0.0
        ...
        muy3 = 1.5 * bspace
        f = Expression(
            "max(max(max(alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/(2*sig0), 2))),  "
            "alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/(2*sig1), 2)))),  "
            "alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/(2*sig2), 2)))),  "
            "alph3*sig3*exp(-0.5*(pow((x[0] - mux3)/sig3, 2)) -0.5*(pow((x[1] - muy3)/(2*sig3), 2))))  ",
            alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3],
            ...
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3]]
    elif surf == 'bumps1x7aspect2max':
        alph = [aval, aval, aval, aval, aval, aval, aval]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        mux0 = 0.0
        mux1 = 0.0
        mux2 = 0.0
        ...
        muy0 = basey + 0.5 * bspace
        muy1 = basey + 1.5 * bspace
        muy2 = basey + 2.5 * bspace
        muy3 = basey + 3.5 * bspace
        muy4 = basey + 4.5 * bspace
        muy5 = basey + 5.5 * bspace
        muy6 = basey + 6.5 * bspace
        f = Expression(
            "max(max(max(max(max(max(alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/(2*sig0), 2))),  "
            "alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/(2*sig1), 2)))),  "
            "alph2*sig2*exp(-0.5*(pow((x[0] - mux2)/sig2, 2)) -0.5*(pow((x[1] - muy2)/(2*sig2), 2)))),  "
            "alph3*sig3*exp(-0.5*(pow((x[0] - mux3)/sig3, 2)) -0.5*(pow((x[1] - muy3)/(2*sig3), 2))))  "
            "alph4*sig4*exp(-0.5*(pow((x[0] - mux4)/sig4, 2)) -0.5*(pow((x[1] - muy4)/(2*sig4), 2))))  "
            "alph5*sig5*exp(-0.5*(pow((x[0] - mux5)/sig5, 2)) -0.5*(pow((x[1] - muy5)/(2*sig5), 2))))  "
            "alph6*sig6*exp(-0.5*(pow((x[0] - mux6)/sig6, 2)) -0.5*(pow((x[1] - muy6)/(2*sig6), 2))))  ",
            alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3], alph4=alph[4], alph5=alph[5], alph6=alph[6],
            mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2, mux3=mux3, muy3=muy3,
            mux4=mux4, muy4=muy4, mux5=mux5, muy5=muy5, mux6=mux6, muy6=muy6,
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3], sig4=x0[4], sig5=x0[5], sig6=x0[6])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3], [mux4, muy4], [mux5, muy5], [mux6, muy6]]
    elif surf == 'bumps1x5':
        alph = [aval, aval, aval, aval, aval]
        # x0 = [(1./2.35)*R,(1./2.35)*R]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        ...
        f = Expression(
            ...
            "alph3*sig3*exp(-0.5*(pow((x[0] - mux3)/sig3, 2)) -0.5*(pow((x[1] - muy3)/sig3, 2)))),  "
            "alph4*sig4*exp(-0.5*(pow((x[0] - mux4)/sig4, 2)) -0.5*(pow((x[1] - muy4)/sig4, 2))))  ",
            alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3], alph4=alph[4],
            mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2, mux3=mux3, muy3=muy3, mux4=mux4,
            muy4=muy4,
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3], sig4=x0[4])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3], [mux4, muy4]]
    elif surf == 'bumps1x5add':
        alph = [aval, aval, aval, aval, aval]
        # x0 = [(1./2.35)*R,(1./2.35)*R]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.8 * R
        ...
        alph = [aval, aval, aval, aval, aval, aval]
        # x0 = [(1./2.35)*R,(1./2.35)*R]
        ...
        muy5 = 2.5 * bspace
        f = Expression(
            "max(max(max(max(max(alph0*sig0*exp(-0.5*(pow((x[0]-mux0)/sig0,2))-0.5*(pow((x[1]-muy0)/sig0,2))), "
            "alph1*sig1*exp(-0.5*(pow((x[0]-mux1)/sig1, 2)) -0.5*(pow((x[1]-muy1)/sig1, 2)))),  "
            "+alph2*sig2*exp(-0.5*(pow((x[0]-mux2)/sig2, 2)) -0.5*(pow((x[1]-muy2)/sig2, 2)))),  "
            "+alph3*sig3*exp(-0.5*(pow((x[0]-mux3)/sig3, 2)) -0.5*(pow((x[1]-muy3)/sig3, 2)))),  "
            "+alph4*sig4*exp(-0.5*(pow((x[0]-mux4)/sig4, 2)) -0.5*(pow((x[1]-muy4)/sig4, 2)))),  "
            "+alph5*sig5*exp(-0.5*(pow((x[0]-mux3)/sig5, 2)) -0.5*(pow((x[1]-muy5)/sig5, 2))))  ",
            alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3], alph4=alph[4], alph5=alph[5],
            mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2, mux3=mux3, muy3=muy3, mux4=mux4,
            muy4=muy4, mux5=mux5, muy5=muy5,
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3], sig4=x0[4], sig5=x0[5])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3], [mux4, muy4], [mux5, muy5]]
    elif surf == 'bumps1x6dodgemax':
        alph = [aval, aval, aval, aval, aval, aval]
        # x0 = [(1./2.35)*R,(1./2.35)*R]
        x0 = [0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R, 0.2 * R]
        bspace = 0.6 * R
        ...
        muy6 = 3.0 * bspace
        f = Expression(
            "max(max(max(max(max(max(alph0*sig0*exp(-0.5*(pow((x[0]-mux0)/sig0,2))-0.5*(pow((x[1]-muy0)/sig0,2))), "
            "alph1*sig1*exp(-0.5*(pow((x[0]-mux1)/sig1, 2)) -0.5*(pow((x[1]-muy1)/sig1, 2)))),  "
            "+alph2*sig2*exp(-0.5*(pow((x[0]-mux2)/sig2, 2)) -0.5*(pow((x[1]-muy2)/sig2, 2)))),  "
            "+alph3*sig3*exp(-0.5*(pow((x[0]-mux3)/sig3, 2)) -0.5*(pow((x[1]-muy3)/sig3, 2)))),  "
            "+alph4*sig4*exp(-0.5*(pow((x[0]-mux4)/sig4, 2)) -0.5*(pow((x[1]-muy4)/sig4, 2)))),  "
            "+alph5*sig5*exp(-0.5*(pow((x[0]-mux5)/sig5, 2)) -0.5*(pow((x[1]-muy5)/sig5, 2)))),  "
            "+alph6*sig6*exp(-0.5*(pow((x[0]-mux6)/sig6, 2)) -0.5*(pow((x[1]-muy6)/sig6, 2))))  ",
            alph0=alph[0], alph1=alph[1], alph2=alph[2], alph3=alph[3], alph4=alph[4], alph5=alph[5], alph6=alph[6],
            mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, mux2=mux2, muy2=muy2, mux3=mux3,
            muy3=muy3, mux4=mux4, muy4=muy4, mux5=mux5, muy5=muy5, mux6=mux6, muy6=muy6,
            sig0=x0[0], sig1=x0[1], sig2=x0[2], sig3=x0[3], sig4=x0[4], sig5=x0[5], sig6=x0[6])
        arrangement = [[mux0, muy0], [mux1, muy1], [mux2, muy2], [mux3, muy3], [mux4, muy4], [mux5, muy5], [mux6, muy6]]
    elif surf == 'bumps2x1':
        alph = [0.70, 0.70]
        ...
        f = Expression("alph0*sig0*exp(-0.5*(pow((x[0] - mux0)/sig0, 2)) -0.5*(pow((x[1] - muy0)/sig0, 2)))  "
                       "+alph1*sig1*exp(-0.5*(pow((x[0] - mux1)/sig1, 2)) -0.5*(pow((x[1] - muy1)/sig1, 2)))  ",
                       alph0=alph[0], alph1=alph[1], mux0=mux0, muy0=muy0, mux1=mux1, muy1=muy1, sig0=x0[0], sig1=x0[1])
        arrangement = [[mux0, muy0], [mux1, muy1]]
    elif surf == 'bumps2x2':
        bspace = R * 0.3
        mux0 = bspace * 0.5
        mux1 = -bspace * 0.5
...
def U_displ_constP_changealpha(nu, U_old, alpha_old, x0, R, alpha_new):
    ...
    alphaterm_old = (alpha_old / 2.) ** 2. * (
    (x0 / R) ** 2. * (np.exp(-R ** 2. / x0 ** 2.) - 1.0) + np.exp(-R ** 2. / x0 ** 2.))
    Pstar = 1. / (1. - nu) * (U_old - alphaterm_old)
    alphaterm_new = (alpha_new / 2.) ** 2. * (
    (x0 / R) ** 2. * (np.exp(-R ** 2. / x0 ** 2.) - 1.0) + np.exp(-R ** 2. / x0 ** 2.))
...
def U_displ_constP_changeR(nu, U_old, R_old, x0, R, alph):
    ...
    # 0.0, 0.2121320343559643, .4242640687119285, 0.7064460135092848
    # returns the new U (displacement)
    alphaterm_old = (alph / 2.) ** 2. * (
    (x0 / R_old) ** 2. * (np.exp(-R_old ** 2. / x0 ** 2.) - 1.0) + np.exp(-R_old ** 2. / x0 ** 2.))
    Pstar = 1. / (1. - nu) * (U_old - alphaterm_old)
    alphaterm_new = (alph / 2.) ** 2. * (
    (x0 / R) ** 2. * (np.exp(-R ** 2. / x0 ** 2.) - 1.0) + np.exp(-R ** 2. / x0 ** 2.))
    U_new = (1. - nu) * Pstar + alphaterm_new
    return U_new
...
def GB_stress_uDirichlet(alph, x0, R, U, nu, t):
    ...
    Srr = alph ** 2 / 8. * (
    (x0 / t) ** 2 * (1. - np.exp(-(t / x0) ** 2)) + (x0 / R) ** 2 * (np.exp(-(R / x0) ** 2) - 1.0)) + P
    Stt = alph ** 2 / 8. * (
    (x0 / t) ** 2 * (np.exp(-(t / x0) ** 2) - 1.) + 2. * np.exp(-(t / x0) ** 2) + (x0 / R) ** 2 * (
    np.exp(-(R / x0) ** 2) - 1.0)) + P
    return Srr, Stt
...
def gaussian_curvature_unstructured(x, y, z, dx, N=3):
    ...
    # print 'shape(yv)=', np.shape(yv)
    # print 'shape(zgrid)=', np.shape(zgrid)
    # print 'shape(lookupXYZ)=', np.shape(lookupXYZ)
    # print 'shape(K)=', np.shape(K)
    # print 'shape(Kv)=', np.shape(Kv)
    # print 'shape(xy_pts)=', np.shape(xy_pts)
    # print 'shape(Kv_out)=', np.shape(Kv_out)
    return K, X, Y, Kgrid
...
def generate_diskmesh(R, n, steps_azim):
    ...
    vals = array([[R * cos(ii * 2 * pi / steps_azim), R * sin(ii * 2 * pi / steps_azim)] for ii in
                  arange(steps_azim)])  # circle points
...
def closest_pts_along_line(pts, endpt1, endpt2):
    ...
    projv = np.dstack((aa * (aa * xx + bb * yy) / (aa ** 2 + bb ** 2),
                       bb * (aa * xx + bb * yy) / (aa ** 2 + bb ** 2)))[0]
...
def line_pt_is_on_lineseg(p, a, b):
    """Check if a point (p) which lies along a line is between two other points (a,b) on that line
    (ie, is on a line segment)
...
def closest_pt_on_lineseg_dolf(pt, endpt1, endpt2):
    ...
    Differs from non-dolf version by using dolfin sqrt (not sure why this should be necessary, but seems to be...)
...
def closest_pt_on_any_lineseg_dolf(pt, linesegs):
    """Get closest point on any linesegment.
    """
    first = True
    for lineseg in linesegs:
        endpt1 = lineseg[0]
        endpt2 = lineseg[1]
        linpt, dist = closest_pt_on_lineseg_dolf(pt, endpt1, endpt2)
        if first:
            outpt = linpt
            outdist = dist
            first = False
        ...
        if OUT[key][0]:
            ...
            if name == 'phase':
                pe.pf_plot_scatter_scalar(x, y, phiv, imdir + name, name, ind, title, title2, subtext, subsubtext,
                                          ptsz=ptsz, vmin=0., vmax=1.0, shape=params['shape'])
            if name == 'contour':
                pe.pf_contour_unstructured(x, y, phiv, 200, 0.2, Bxy, imdir + name, name, ind, title, title2, subtext,
                                           subsubtext)
            if name == 'gstrain':
                gstrain = project(
                    (4 * phi ** 3 - 3 * phi ** 4) * dolf.tr(dolf.sym(nabla_grad(u)) + 0.5 * outer(grad(h), grad(h))),
                    Vf)
                gstrainv = gstrain.vector().array()
                pe.pf_plot_scatter_scalar(x, y, gstrainv, imdir + name, name, ind, title, title2, subtext, subsubtext,
                                          ptsz=ptsz, shape=params['shape'])
            if name == 'displacement':
                pe.pf_plot_scatter_2panel(x, y, uxv, uyv, imdir + name, name, 'u', ind, title, title2, subtext,
                                          subsubtext, shape=params['shape'])
            if name == 'energy':
                En_V = project((4 * phi ** 3 - 3 * phi ** 4) * (
                    E * nu / (2 * (1 - nu ** 2)) * dolf.tr(
                        dolf.sym(nabla_grad(u)) + 0.5 * outer(grad(h), grad(h))) ** 2 + \
                    mu * inner(dolf.sym(nabla_grad(u)).T + 0.5 * outer(grad(h), grad(h)).T,
                               dolf.sym(nabla_grad(u)) + 0.5 * outer(grad(h), grad(h)))), Vf)
                Env = En_V.vector().array()
                pe.pf_plot_scatter_scalar(x, y, Env, imdir + name, name, ind, title, title2, subtext, subsubtext,
                                          vmin=0, vmax=Ec, ptsz=ptsz, shape=params['shape'])
            if name == 'phidiff':
                if ii > 0 and firstpass == 0:
                    dphi = phiv - phiv_old
                    limC = np.nanmax(np.abs(dphi))
                    pe.pf_plot_scatter_scalar(x, y, dphi, imdir + name, name, ind - 1, title, title2, subtext,
                                              subsubtext, ptsz=ptsz, shape=params['shape'], vmin=-limC, vmax=limC,
                                              cmap=cm.coolwarm)
            ...
            if name == 'gstress':
                ...
                s_V = project((4 * phi ** 3 - 3 * phi ** 4) * (
                    E / (1 + nu) * (epsilon_k) + E * nu / ((1 - nu ** 2)) * dolf.tr(epsilon_k) * dolf.Identity(d)), Vt)
                sxx = project(s_V[0, 0], Vf)
                sxxv = sxx.vector().array()
                sxy = project(s_V[0, 1], Vf)
                sxyv = sxy.vector().array()
                syx = project(s_V[1, 0], Vf)
                syxv = syx.vector().array()
                syy = project(s_V[1, 1], Vf)
                syyv = syy.vector().array()
                if vminmax == 'default':
                    ...
                    vmin = 'auto'
                    vmax = 'auto'
                pe.pf_plot_scatter_4panel(x, y, sxxv, sxyv, syxv, syyv, imdir + name, name, r'\sigma', ind, title,
                                          title2, subtext, subsubtext,
                                          vmin=vmin, vmax=vmax, cmap='seismic', shape=params['shape'],
                                          ptsz=np.max(ptsz * 0.05, 0.1))
...
def pf_display_scalar(x, y, C, title, fig=None, ax=None, cbar=None, cbar_on=True, vmin='auto', vmax='auto',
                      ptsz=5, cmap=cm.CMRmap, hilite='none', axis_on=True, show=True, close=True, shape='circle',
                      R=None, xylim=None):
    ...
    if shape == 'circle':
        ...
        t = np.array([-R, R])
        ax.plot(R * np.array([1, 1]), t, 'k-')
        ax.plot(t, R * np.array([1, 1]), 'k-')
        ax.plot(t, -R * np.array([1, 1]), 'k-')
        ax.plot(-R * np.array([1, 1]), t, 'k-')
    elif shape == 'unitsq':
        t = np.array([0, R])
        ax.plot(R * np.array([1, 1]), t, 'k-')
        ax.plot(t, R * np.array([0, 1]), 'k-')
        ax.plot(t, -R * np.array([0, 1]), 'k-')
        ax.plot(np.array([0, 0]), t, 'k-')
    elif shape == 'rectangle2x1':
        t = np.array([-R, R])
        ax.plot(R * np.array([1, 1]), 2 * t, 'k-')
        ax.plot(t, 2 * R * np.array([1, 1]), 'k-')
        ax.plot(t, -2 * R * np.array([1, 1]), 'k-')
        ax.plot(-R * np.array([1, 1]), 2 * t, 'k-')
...
def pf_titles(field, params, t):
    ...
    if 'BCUP' in params:
        if params['BCUP'] == 'essential':
            if params['surf'] in ['bump', 'monkeysaddle', 'hyperbolic-paraboloid', 'hyperbolic-paraboloid-transp']:
                title2 = params['surf'] + r': $x_0$=' + '{0:.3f}'.format(params['x0']) + \
                         r' $\alpha$=' + '{0:.3f}'.format(params['alph']) + \
                         r' $U=$' + '{0:.3f}'.format(params['U']) + \
                         ' BC=' + params['BCtype'] + r' $R=$' + str(params['L'] / 2) + \
                         r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi)
                HRx0str = r' $H/x_0$=' + '{0:.3f}'.format(params['H'] / params['x0'])
            elif params['surf'] == 'flat':
                title2 = r'Flat: $U=$' + '{0:.3f}'.format(params['U']) + ' BC=' + params['BCtype'] + r' $R=$' + str(
                ...
                title2 = r'Corrugated: $U=$' + '{0:.3f}'.format(params['U']) + ' BC=' + params[
                    'BCtype'] + r' $R=$' + str(params['L'] / 2) + \
                         ...
                         r' $x_0/R$=[' + '{0:0.2f}'.format(params['x0'][0] * 2 / params['L']) + ',' + '{0:0.2f}'.format(
                    params['x0'][1] * 2 / params['L']) + ']' + \
                         r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi)
                HRx0str = r' $H/R$=' + '{0:.3f}'.format(params['H'] * 2. / params['L'])
                ...
                title2 = params['surf'] + r': $x_0$=' + '{0:.3f}'.format(params['x0']) + \
                         r' $\alpha$=' + '{0:.3f}'.format(params['alph']) + \
                         r' $P=$' + '{0:.3f}'.format(params['U']) + \
                         'E BC=' + params['BCtype'] + r' $R=$' + str(params['L'] / 2) + \
                         r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi)
                HRx0str = r' $H/R$=' + '{0:.3f}'.format(params['H'] * 2. / params['L'])
        elif params['BCUP'] == 'natural':
            if params['surf'] == 'bump':
                title2 = r'Bump: $x_0$=' + '{0:.3f}'.format(params['x0']) + r' $\alpha$=' + '{0:.3f}'.format(
                    params['alph']) + r' $P=$' + '{0:.3f}'.format(params['U']) + 'E BC=' + params[
                             'BCtype'] + r' $R=$' + str(params['L'] / 2) + \
                         r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi)
                HRx0str = r' $H/x_0$=' + '{0:.3f}'.format(params['H'] / params['x0'])
            elif params['surf'] == 'flat':
                title2 = r'Flat: $P=$' + '{0:.3f}'.format(params['U']) + 'E BC=' + params['BCtype'] + r' $R=$' + str(
                    params['L'] / 2) + r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi)
                HRx0str = r' $H/R$=' + '{0:.3f}'.format(params['H'] * 2. / params['L'])
            elif params['surf'] == 'bumps2x1':
                print(r' x0=[' + '{0:0.2f}'.format(params['x0'][0]) + ',' + '{0:0.2f}'.format(params['x0'][1]) + ']')
                ...
                title2 = params['surf'] + r': $x_0$=' + '{0:.3f}'.format(
                    params['x0']) + r' $\alpha$=' + '{0:.3f}'.format(params['alph']) + r' $P=$' + '{0:.3f}'.format(
                    params['U']) + 'E BC=' + params['BCtype'] + r' $R=$' + str(params['L'] / 2) + \
                         r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi)
                HRx0str = r' $H/R$=' + '{0:.3f}'.format(params['H'] * 2. / params['L'])
        elif params['BCUP'] == 'natural-essential':
            if params['surf'] == 'bump':
                title2 = r'Bump: U=0 and $x_0$=' + '{0:.3f}'.format(params['x0']) + r' $\alpha$=' + '{0:.3f}'.format(
                    params['alph']) + r' $P=$' + '{0:.3f}'.format(params['U']) + 'E BC=' + params[
                             'BCtype'] + r' $R=$' + str(params['L'] / 2) + \
                         r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi)
                HRx0str = r' $H/x_0$=' + '{0:.3f}'.format(params['H'] / params['x0'])
            elif params['surf'] == 'flat':
                title2 = r'Flat: U=0 and $P=$' + '{0:.3f}'.format(params['U']) + 'E BC=' + params[
                    'BCtype'] + r' $R=$' + str(params['L'] / 2) + r' $\theta=$' + '{0:.3f}'.format(
                    params['theta'] / np.pi)
                HRx0str = r' $H/R$=' + '{0:.3f}'.format(params['H'] * 2. / params['L'])
            else:
                title2 = params['surf'] + r': U=0 and $P=$' + '{0:.3f}'.format(params['U']) + \
                         'E BC=' + params['BCtype'] + r' $R=$' + \
                         str(params['L'] / 2) + r' $\theta=$' + '{0:.3f}'.format(params['theta'] / np.pi) + \
    ...
    subtext = r'$\xi$=' + '{0:3f}'.format(params['xi']) + ' $\gamma$=' + '{0:3f}'.format(params['gamma']) + \
              r' $\tau$=' + '{0:03f}'.format(params['Tau'])
    subsubtext = r'E=' + str(params['E']) + r' $\nu$=' + '{0:.2f}'.format(params['nu']) + \
                 r' $E_c$=' + '{0:.4f}'.format(params['Ec']) + \
...
def pf_title_QGP(surf, P, DT, decayL, coldL, L):
    return surf + r' $P=$' + str(P) + r' $\Delta T$=' + str(DT) + r' decayL=' + str(decayL) + r' coldL=' + str(
        coldL) + r' $L=$' + str(L)


def title_scalar_static(field, params):
    return field + r': L/$\xi$=' + '{0:.3f}'.format(params['L'] / params['xi']) + \
           r' n/$\xi$=' + '{0:.3f}'.format(params['meshd']) + \
           r' $\eta=$' + '{0:.3f}'.format(params['eta'])


def pf_titles_static(field, params):
    title = title_scalar_static(field, params)
    if params['surf'] == 'bump':
        title2 = r'Bump: $x_0$=' + str(params['x0']) + r' $\alpha$=' + str(params['alph']) + r' $U=$' + str(
            params['U']) + ' BC=' + params['BCtype'] + r' $R=$' + str(params['L'] / 2)
    elif params['surf'] == 'flat':
        title2 = r'Flat: $U=$' + str(params['U']) + ' BC=' + params['BCtype'] + r' $R=$' + str(params['L'] / 2)
    elif params['surf'] == 'sphere':
        title2 = r'Sphere: $x_0$=' + str(params['x0']) + r' $\alpha$=' + str(params['alph']) + r' $U=$' + str(
            params['U']) + ' BC=' + params['BCtype'] + r' $R=$' + str(params['L'] / 2)
    else:
        title2 = r'$x_0$=' + str(params['x0']) + r' $\alpha$=' + str(params['alph']) + r' $U=$' + str(
            params['U']) + ' BC=' + params['BCtype'] + r' $R=$' + str(params['L'] / 2)
